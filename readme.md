...
```
dbx execute --cluster-name=<name of interactive cluster> --job=ad_attrib-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=ad_attrib-sample-integration-test --files-only
dbx launch --job=ad_attrib-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `ad_attrib/__init__.py` file and tag the current code version:
```
